...
def ask_gemini(prompt):
    try:
        if not GEMINI_KEY: return "❌ Erreur: Clé Manquante."
        
        # AVEC LA NOUVELLE LIBRAIRIE, CE NOM FONCTIONNE PARFAITEMENT
...
def send_welcome(message):
    bot.reply_to(message, "👋 Bot mis à jour & prêt !")
...
def force_analyze(message):
    bot.reply_to(message, "🧠 Analyse Flash en cours...")
    analyze_market(manual_trigger=True, chat_target=message.chat.id)

@bot.message_handler(func=lambda message: True)
def echo_all(message):
    bot.send_chat_action(message.chat.id, 'typing')
    reply = ask_gemini(f"Expert crypto, réponds court : {message.text}")
...
def analyze_market(manual_trigger=False, chat_target=None):
    ...
    prompt = f"""
    Agis comme un trader pro. Analyse ces clôtures BTC (15m): {closes[-5:]}.
...
def run_bot_safe():
    # C'est ici qu'on tue l'erreur 409
    try:
        bot.remove_webhook()
        time.sleep(2)
    except: pass
